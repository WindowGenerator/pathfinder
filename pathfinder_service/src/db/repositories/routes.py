...
from src.db.models import Coordinates, Routes
from src.schemas.routes import Route as RouteSchema, ReportPerUser as ReportPerUserSchema
from src.schemas.coordinates import CoordinateID as CoordinateIDSchema, Coordinate as CoordinateSchema
...
class RoutesRepository:
    ...
    async def get_routes(self) -> List[RouteSchema]:
        result = await self._session.execute(select(Routes).options(selectinload(Routes.coordinates)))
        routes = []
    ...
    async def save_route(self, created_by_user_with_id: int, coordinates: List[CoordinateIDSchema]) -> RouteSchema:
        route_id = str(uuid4())

        coordinate_ids = [coordinate.id for coordinate in coordinates]

        route = Routes(
            id=route_id,
            created_by_user_with_id=created_by_user_with_id,
            coordinates=await self._coord_repo.get_raw_coordinates_by_ids(coordinate_ids),
    ...
    async def get_route_by_id(self, route_id: str) -> RouteSchema:
        result = await self._session.execute(
            select(Routes).where(Routes.id == route_id).options(selectinload(Routes.coordinates))
        )
        await self._session.commit()
        route = result.scalar()
    ...
    def _convert_route_from_db(self, route) -> RouteSchema:
        ...
        return RouteSchema(
            ...
            coordinates=[coordinates[str(route_coord_id)] for route_coord_id in route.route_order],
    ...
    async def get_reports(self) -> List[ReportPerUserSchema]:
        ...
        for route in routes:
            ...
            for coord in route.coordinates:
                ...
                route_length += math.sqrt((previous_coord.x_coord - coord.x_coord) ** 2 + (previous_coord.y_coord - coord.y_coord) ** 2)

                previous_coord = coord

            user_to_report[route.created_by_user_with_id]["routes_length"] += route_length
